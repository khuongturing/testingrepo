...
CREATE PROCEDURE catalog_get_products(IN inShortProductDescriptionLength INT,
...
-- Create catalog_add_product stored procedure
CREATE PROCEDURE catalog_add_product(
  IN inName VARCHAR(100), IN inDescription VARCHAR(1000), IN inPrice DECIMAL(10,2), IN inDiscountedPrice DECIMAL(10,2), IN inImage VARCHAR(150), IN inImage2 VARCHAR(150), IN inThumbnail VARCHAR(150), IN inDisplay SMALLINT(6))
BEGIN
  INSERT INTO department (name, description, price, discounted_price, image, image_2, thumbnail, display)
         VALUES (inName, inDescription, inPrice, inDiscountedPrice, inImage, inImage2, inThumbnail, inDisplay);
...
CREATE PROCEDURE catalog_update_category(IN inCategoryId INT,
  IN inName VARCHAR(100), IN inDescription VARCHAR(1000), IN inDepartmentId INT)
BEGIN
    UPDATE category
    SET    name = inName, description = inDescription, department_id = inDepartmentId
    WHERE  category_id = inCategoryId;
END$$
...
CREATE PROCEDURE catalog_get_categories(IN inOrderBy VARCHAR(20), IN inCategoriesPerPage INT, IN inStartItem INT)
BEGIN
  PREPARE statement FROM
    "SELECT  *
    FROM     category
...
-- Create tax_get_taxes_list stored procedure
CREATE PROCEDURE tax_get_taxes_list()
BEGIN
  SELECT * 
  FROM tax 
  ORDER BY tax_id;
...
-- Create tax_get_tax stored procedure
CREATE PROCEDURE tax_get_tax(IN inTaxId INT)
BEGIN
  SELECT * 
  FROM tax 
...
-- Create tax_get_tax stored procedure
CREATE PROCEDURE customer_get_shipping_region(IN inShippingRegionId INT)
BEGIN
  SELECT * 
